...
for (num = 0; num < 99; num++)
{
	    ...
	    fprintf(gp,"splot '%s' binary filetype=png with rgbimage,'%s'  using ($1*16+32):($2*16+32):3 with pm3d, '%s' using ($1*16+32):($2*16+32):($1*0.0):(%lf*$3):(%lf*$4):($1*0.0) with vectors head filled lt 2 lc 'black' \n",read_back_img,read_file,read_file,v_r,v_r);
