...
unsigned char image_out[times][height][width];

double a[N],b[N];
double u[N],v[N];
double radius[N]; // Each particle diameter
int particle_x[times][N],particle_y[times][N];
double Z; // Standard Normal Distribution
double bright[times][height][width];
...
int main (){
  ...
  for(int c=0; c < times ; c++){
    ...
    for (int j = 0; j < height; j++){
      for (int k = 0; k < width; k++){
        image_out[c][j][k] = (bright[c][j][k]-min[c])/(max[c]-min[c])*255;
      }
    }
  ...
  while (cw < times)
  {
    sprintf(outputpath,"%s/%05d.bmp",outputheader,cw);
    fp = fopen(outputpath, "wb");
    fwrite(header_buf, sizeof(unsigned char), 1078, fp);// Write Header
    fwrite(image_out, sizeof(unsigned char), 1024*1024, fp);// Write 8bit image intensity
    fclose (fp);
    cw++;
  }
