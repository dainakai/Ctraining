...
char output_data_file[100]; // name of output file
const int tmax=100;
...
int main (){
  ...
  {
    int i,j;
    sprintf(output_data_file,"./results1/%06d.dat",c);
    ...
    for(i =0;i< x_grid ;i++){
      for(j=0;j< y_grid ;j++){
        //Taylor-Green vortex
        u[i][j]= exp(-2.0*c)*cos (2.0* PI/ x_grid *i)* sin (2.0* PI/ y_grid *j);
        v[i][j]= -1 *exp(-2.0*NU*c)* sin (2.0* PI/ x_grid *i)*cos (2.0* PI/ y_grid *j);
        omega[i][j]= -2*exp(-2.0*NU*c)* cos (2.0* PI/ x_grid *i)*cos (2.0* PI/ y_grid *j);
  ...
  for (int c = 0; c < tmax; c++)
  {
   if ((gp = popen("gnuplot", "w")) == NULL) {
    ...
    //PNG image
  	...
  	fprintf(gp,"set output './results1png/%06d.png'\n",c);
    ...
    fprintf(gp,"splot './results1/%06d.dat'  using 2:1:3 with pm3d, './results1/%06d.dat' using 2:1:($1*0.0): (%lf*$5):(%lf*$4):($1*0.0) with vectors head filled lt 2 lc 'black' \n",c,c,v_r,v_r);
