...
void twoDimFFT(double *re, double *im, int height, int width);
...
int main(){
    ...
    for (int i = 0; i < height; i++){
        for (int j = 0; j < width; j++){
            *(image_in + i*height + j) = (unsigned char)(127.5*sin(2*M_PI*10.0*(i)/width)+127.5);
        }
    }

    fp = fopen(outputimage1,"wb");
    fwrite(header_buf,sizeof(unsigned char),1078,fp);
    fwrite(image_in,sizeof(unsigned char),width*height,fp);
    fclose(fp);
    ...
    twoDimFFT(re_X, im_X, height, width);
...
void twoDimFFT(double *re, double *im, int height, int width){
    ...
    for (int i = 0; i < height; i++){
        ...
        S_fft(re_temp1, im_temp1, width, 1);
        for (int j = 0; j < width; j++){
    ...
    for (int i = 0; i < width; i++){
        ...
        S_fft(re_temp2, im_temp2, height, 1);
        for (int j = 0; j < height; j++){
